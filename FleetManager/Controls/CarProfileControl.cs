...
namespace FleetManager.Controls
{
    public partial class CarProfileControl : UserControl
    {
        ...
        public CarProfileControl(byte[] token, MainMenu mainMenu, string username)
        {
            InitializeComponent();

            DataTable carTable = SqlConn.GetTableProcedure("PROC_GET_CAR_PROFILE", "input_username", username, token);
            if (carTable == null)
                mainMenu.ExitProgram();
            else
            {
